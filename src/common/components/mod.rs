...
#[component]
pub fn Alert(alert_type: String, msg: Signal<Option<String>>) -> impl IntoView {
    let alert_type = Signal::derive(move || alert_type.clone());
    let symbol = Signal::derive(move || {
        match AlertType::from_str(alert_type().as_ref()).expect("Can't parse alert type") {
            AlertType::None => {
                view! {
                    <svg
    ...
    let alert_class = Signal::derive(move || {
        match AlertType::from_str(alert_type().as_ref()).expect("Can't parse alert type") {
    ...
    view! {
        <Show when=show_alert>
            <div class=move || alert_class()>{symbol()} <span>{msg()}</span></div>
        </Show>
    }
}
